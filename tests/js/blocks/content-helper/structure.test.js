...
import { PARSELY_API_BASE_URL } from '../../../../src/blocks/shared/utils/constants';
...
describe( 'Content Helper', () => {
	...
	test( 'should show a single top post with description and proper attributes', async () => {
		...
		expect( statsLink.getAttribute( 'href' ) ).toEqual( `${ PARSELY_API_BASE_URL }/example.com/post-1` );
	...
	function getRelatedTopPostsMockData( postsCount = RELATED_POSTS_DEFAULT_LIMIT ) {
		...
		for ( let i = 1; i <= postsCount; i++ ) {
			posts.push( {
				...
				statsUrl: `${ PARSELY_API_BASE_URL }/example.com/post-${ i }`,
				title: `Title ${ i }`,
