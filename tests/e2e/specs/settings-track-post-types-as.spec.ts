...
describe( 'Track Post Types as', () => {
	...
	it( 'Should be able to save non-default selections', async () => {
		...
		expect( await page.$eval( radioPostAsPost, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPostAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPostAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();

		// Verify that page is tracked as post
		expect( await page.$eval( radioPageAsPost, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();
		expect( await page.$eval( radioPageAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPageAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();

		// Verify that attachment is tracked as page
		expect( await page.$eval( radioAttachmentAsPost, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioAttachmentAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();
		expect( await page.$eval( radioAttachmentAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
	} );
	...
	it( 'Should be able to save everything as none', async () => {
		...
		expect( await page.$eval( radioPostAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPostAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();
		expect( await page.$eval( radioPageAsPost, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPageAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPageAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();
		expect( await page.$eval( radioAttachmentAsPost, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioAttachmentAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioAttachmentAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();
	} );
	...
	it( 'Should be browsable with arrow and tab keys', async () => {
		...
		expect( await page.$eval( radioPostAsPost, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();
		expect( await page.$eval( radioPostAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPostAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPageAsPost, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioPageAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();
		expect( await page.$eval( radioPageAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioAttachmentAsPost, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioAttachmentAsPage, ( input: HTMLInputElement ) => input.checked ) ).toBeFalsy();
		expect( await page.$eval( radioAttachmentAsNone, ( input: HTMLInputElement ) => input.checked ) ).toBeTruthy();
