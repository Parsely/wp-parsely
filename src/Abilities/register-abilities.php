...
function register_parsely_abilities(): void {
    ...
    $permission_cb = static function ( $input = null ) {
    ...
    wp_register_ability( 'wp-parsely/get-post-analytics', array(
        ...
        'execute_callback' => static function ( $input ) {
            ...
            if ( empty( $url ) && isset( $input['post_id'] ) ) {
    ...
    // Get Post Referrers
    wp_register_ability( 'wp-parsely/get-post-referrers', array(
    ...
    // Get Related Posts
    wp_register_ability( 'wp-parsely/get-related', array(
    ...
    wp_register_ability( 'wp-parsely/get-analytics-posts', array(
        ...
        'input_schema' => array(
            'type' => 'object',
            'properties' => array(
                'period_start' => array( 'type' => 'string', 'description' => 'Start of period (YYYY-MM-DD or Nd).' ),
                'period_end' => array( 'type' => 'string', 'description' => 'End of period (YYYY-MM-DD).' ),
                'pub_date_start' => array( 'type' => 'string' ),
                'pub_date_end' => array( 'type' => 'string' ),
                'sort' => array( 'type' => 'string', 'description' => 'Sort metric, e.g., views, visitors, avg_engaged.' ),
                'limit' => array( 'type' => [ 'integer', 'string' ], 'description' => 'Max results (number or "max").' ),
                'tag' => array( 'type' => 'array', 'items' => array( 'type' => 'string' ), 'description' => 'Filter by tag(s).' ),
                'urls' => array( 'type' => 'array', 'items' => array( 'type' => 'string', 'format' => 'uri' ), 'description' => 'Filter by specific URL(s).' ),
            ),
            'additionalProperties' => false,
        ),
        'output_schema' => array(
    ...
    wp_register_ability( 'wp-parsely/validate-credentials', array(
    ...
    wp_register_ability( 'wp-parsely/get-dash-url', array(
        ...
        'output_schema' => array(
            'type' => 'object',
            'properties' => array(
                'dash_url' => array( 'type' => 'string', 'format' => 'uri' ),
            ),
        ),
        'execute_callback' => static function ( $input ) {
            $parsely = \Parsely\get_parsely();
