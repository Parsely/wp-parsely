...
class Cross_Linker extends Content_Helper_Feature {
	/**
	 * Constructor.
	 *
	 * @since 3.13.0
	...
	public static function get_feature_filter_name(): string {
		return self::get_global_filter_name() . '_cross_linker';
	}
	...
	public static function get_script_id(): string {
		return 'wp-parsely-content-helper-cross-linker';
	}
	...
	/**
	 * Inserts Content Helper Cross Linker inline scripts.
	 *
	 * @since 3.13.0
	 */
	public function run(): void {
		...
		$asset_php        = get_asset_info( 'build/content-helper/cross-linker.asset.php' );
		$built_assets_url = plugin_dir_url( PARSELY_FILE ) . 'build/content-helper/';

		wp_enqueue_script(
			static::get_script_id(),
			$built_assets_url . 'cross-linker.js',
		...
		wp_enqueue_style(
			static::get_style_id(),
			$built_assets_url . 'cross-linker.css',
			array(),
			$asset_php['version']
		);
	}
