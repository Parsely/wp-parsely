...
The `rendered` field is a convenience field is only included if the `wp_parsely_enable_rest_rendered_support` filter returns `true`. To disable this, return `false` from the filter.
