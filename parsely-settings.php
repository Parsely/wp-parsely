<div class="wrap">
    ...
    <form name="parsely" method="post" action="">
        ...
        <table class="form-table">
            <tr valign="top">
                <th scope="row"><label for="apikey"><? _e('API Key / Parse.ly Site:'); ?></label></th>
                <td>
                    <input type="text" name="apikey" id="apikey" value="<? echo $options["apikey"]; ?>" size="20" placeholder="test.com"/>
                    <p class="description">
            ...
            <tr valign="top">
                <th scope="row"><label for="tracker_implementation"><? _e('Tracker Implementation'); ?></label></th>
                <td>
                    <select name="tracker_implementation" id="tracker_implementation">
                        <option value="standard" <? if ($options["tracker_implementation"] == 'standard') { echo 'selected'; }?> >Standard</option>
                        <option value="dom_free" <? if ($options["tracker_implementation"] == 'dom_free') { echo 'selected'; }?>>DOM-Free</option>
                        <option value="async"    <? if ($options["tracker_implementation"] == 'async') { echo 'selected'; }?>>Asynchronous</option>
                    </select>
                    <p class="description">
